<?php
	...
	*/
	...
	/*
		...
		@db: 数据库信息
	*/
    query('../data/tabulated_data.json',$p2); //   ------------------------------------需改数据路径
    ...
    function query($url, $db){
	    ...
	    // 获取表单字段及详情
    	$temp = mysqli_query($conn,'SHOW FULL FIELDS FROM onejson');//json：表名      ---------------------需改表名
		...
		foreach ($my_file as $num => $dataObj) {
			...
			// 添加进数据库     json：表名
			$temp = "INSERT INTO onejson($fieldStr) VALUES($priceStr);";//---------------------需改表名
